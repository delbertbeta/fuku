...
- **user-authentication**: User registration, login, and session management depend on database storage
- **clothing-management**: Clothing item management depends on database operations
- **outfit-management**: Outfit management depends on database operations
- **category-management**: Category management depends on database operations
...
| --------------- | ------ | ----------------------------- | --------------------------------------------------------------------------------- |
| `DATABASE_TYPE` | string | `sqlite`                      | Database type (`sqlite` or `mariadb`)                                             |
| `DATABASE_PATH` | string | `./data/outfit-platform.db`   | SQLite database file path                                                         |
| `MARIADB_URL`   | string | (required when using MariaDB) | MariaDB connection string, format: `mysql://username:password@host:port/database` |

## Type Mappings

| SQLite Type   | MariaDB Type        | Application Type        |
| ------------- | ------------------- | ----------------------- |
| INTEGER       | INT                 | number                  |
...
1. **Configuration Validation**: The system must validate that all required environment variables are present and valid for the selected database type
2. **Connection Management**: The system must properly manage database connections and connection pools to avoid resource leaks
3. **SQL Injection Prevention**: All database operations must use parameterized queries to prevent SQL injection
4. **Error Handling**: The system must provide clear error messages for connection failures, query errors, and constraint violations
5. **Type Consistency**: Data types must be consistent across SQLite and MariaDB to ensure application logic works correctly
...
- `mariadb`: MariaDB Connector/Node.js driver
- `@types/mariadb`: TypeScript type definitions for mariadb
